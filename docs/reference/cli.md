...
Profile `env_file` entries are resolved relative to the spec location (or the current working directory when streaming a spec from stdin). Values such as `../env/app.env` and `~/service.env` are expanded before validation, so specs remain portable across checkouts. See `examples/socialsense/envkeep.toml` for a larger multi-profile example that relies on relative paths into another repository checkout.
